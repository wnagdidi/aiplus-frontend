 ...
 * @returns 处理后的事件数据
 */
export const handleStartTrialEvent = async (session: Session | null, data: any) => {
  try {
    ...
    if (data.fbc && process.env.NEXT_PUBLIC_PIXEL_ID) {
      ...
      await import('react-facebook-pixel')
        ...
        .then((ReactPixel) => {
          ...
          console.log('🟢 [FB埋点] startTrial - 调用 fbq:', {
            method: 'track',
            event: data.event,
            data: fbqData
          })
          ReactPixel.fbq('track', data.event, fbqData)
          console.log('🟢 [FB埋点] startTrial - fbq 调用完成')
        })
        .catch((error) => {
          console.error('🔴 [FB埋点] startTrial - fbq 调用失败:', error)
        })
      // 创建用户行为
      ...
      console.warn('⚠️ [FB埋点] startTrial - 跳过发送:', {
