...
// Deprecated, remove this page after one month.
export default function TermsAndConditionsPage() {
  ...
  const termsData =
    process.env.NEXT_PUBLIC_BRAND_NAME === Brand.AvoidAI
      ...
      : [
          {
            items: [t('terms_of_use_desc_1'), t('terms_of_use_desc_2')],
          },
          {
          ...
          {
            title: t('subscription_services_title'),
            items: [
              t('subscription_services_items_1'),
              t('subscription_services_items_2'),
              t('subscription_services_items_3'),
              t('subscription_services_items_4'),
            ],
          },
          {
            title: t('conduct_and_acceptable_use_policy_title'),
            items: [
              t('conduct_and_acceptable_use_policy_items_1'),
              t('conduct_and_acceptable_use_policy_items_2'),
              t('conduct_and_acceptable_use_policy_items_3'),
              t('conduct_and_acceptable_use_policy_items_4'),
              t('conduct_and_acceptable_use_policy_items_5'),
            ],
          },
          {
            title: t('intellectual_property'),
            items: [
              t('intellectual_property_1'), 
              t('intellectual_property_2'),
              t('intellectual_property_3')
            ],
          },
          {
            title: t('disclaimer_of_warranties_title'),
            items: [
              t('disclaimer_of_warranties_items_1')
            ],
          },
          {
    ...
    <>
      ...
      <div className="bg-background text-[#375375]">
        <div className="max-w-6xl mx-auto px-4 pb-12 pt-24">
          <h1 className="text-3xl mt-8 font-bold text-center text-[36px]">{t('terms_and_conditions')}</h1>
          ...
          {termsData.map((section) => (
            <div key={section.title || section.highlight || section.header} className="mb-6">
              ...
              {section.highlight && (
                <p className="font-semibold mb-3">{section.highlight}</p>
              )}
              <div className="flex flex-col gap-3">
                {section.items?.map((text: string | string[]) =>
                  typeof text === 'string' ? (
                    <p key={text} className="text-default-600">{text}</p>
                  ) : (
                    text.map((listItemText: string) => (
                      <p key={listItemText} className="text-default-600">{listItemText}</p>
                    ))
                  ),
                )}
              </div>
