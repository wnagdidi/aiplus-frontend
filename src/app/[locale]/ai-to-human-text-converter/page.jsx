...
export async function generateMetadata() {
  const t = await getTranslations('Metadata.aiHuman')
  const locale = await getLocale()
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://byecheck.com'
  const canonicalPath = locale === 'en' ? '/ai-to-human-text-converter' : `/${locale}/ai-to-human-text-converter`

  // Generate alternate links for each language
  const languageAlternates = {}
  localesWithName.forEach(({ locale: lang }) => {
    const path = lang === 'en' ? '/ai-to-human-text-converter' : `/${lang}/ai-to-human-text-converter`
    languageAlternates[lang] = `${baseUrl}${path}`
  })
  // Add x-default alternate link
  languageAlternates['x-default'] = `${baseUrl}/ai-to-human-text-converter`
  ...
  return (
    <>
      ...
      <div className="main-wrapper">
        <div className="relative overflow-hidden pb-2" style={{
